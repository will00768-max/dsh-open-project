...
| 下拉列表 | 一个美化后的弹层，**第一行固定为「📂 打开项目文件夹」**，点击用操作系统的文件管理器打开项目文件夹（Windows 为 `explorer.exe`、macOS 为 `open`、Linux 为 `xdg-open`）。其下每个检测到的应用一行，带真实产品图标、名称，并在当前选中的应用上打勾。检测中显示加载占位；无匹配时显示「未检测到可用应用」。 |
