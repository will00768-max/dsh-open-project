// Browser half of the dsh-open-with plugin.
//
...
// editor/terminal with its product icon. Choosing one launches it with the
// current project folder (the session's cwd) and remembers the choice, so the
// icon switches to that app.

const LS_KEY = 'dsh.open-with.last'
...
function OpenWithMenu(props) {
  ...
  const triggerIcon = active
    ? React.createElement('img', { className: 'dsw-openwith-triggerimg', src: active.icon, alt: active.label })
        ...
        : React.createElement('div', { className: 'dsw-openwith-list' },
            apps.map((app) => {
              ...
              return React.createElement('button', {
                ...
                React.createElement('img', { className: 'dsw-openwith-iconimg', src: app.icon, alt: '' }),
